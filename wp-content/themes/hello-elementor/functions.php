...
// function login_filter_callback( $example ) {
// 	$location = $_SERVER['HTTP_REFERER'];
// 	return $location;
//     // return "https://conferences.unite.un.org/athenaknowledgehub/mandates/";
// }
// add_filter( 'openid-connect-generic-cookie-redirect-url', 'login_filter_callback',99 );

add_action( 'wp', 'sc_capture_before_login_page_url',10 );
...
if( !function_exists('sc_after_login_redirection') ):
   ...
   add_filter('openid-connect-generic-cookie-redirect-url', 'sc_after_login_redirection',99);
endif;
...
function new_language_attributes($lang){
	global $post;
    if(is_single()) {
        $ar = get_the_terms($post->ID,"language_attributes");
		if(!empty($ar)){
        foreach($ar as $c) {
            return "lang=\"$c->slug\"";
        }
